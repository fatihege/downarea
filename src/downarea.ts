...
const repeat: Function = (string: string, count: number) => {
    let result: string = '';
...
class DownArea {
    public static RESIZE_OFF: number = 0;
    public static RESIZE_VERTICAL: number = 1;
    public static RESIZE_HORIZONTAL: number = 2;
    public static RESIZE_BOTH: number = 3;
    ...
    private textareaValue: string | number;
    private resize: number;
    private tools: HTMLDivElement[] = [];

    private hiddenTools: string[];

    private actions = {
        ['heading-1']: {fn: this.addHeading},
        ['heading-2']: {fn: this.addHeading, args: [2]},
        ['heading-3']: {fn: this.addHeading, args: [3]},
        ['heading-4']: {fn: this.addHeading, args: [4]},
        ['heading-5']: {fn: this.addHeading, args: [5]},
        ['heading-6']: {fn: this.addHeading, args: [6]},
        ['bold']: {fn: this.addBold},
        ['italic']: {fn: this.addItalic},
        ['bold-italic']: {fn: this.addBoldItalic},
        ['normal-link']: {fn: this.addLink},
        ['quick-link']: {fn: this.addLink, args: [1]},
        ['email']: {fn: this.addEmail},
        ['image']: {fn: this.addImage},
        ['blockquote']: {fn: this.addBlockquote},
        ['u-list']: {fn: this.addUnorderedList},
        ['o-list']: {fn: this.addOrderedList},
        ['sl-code']: {fn: this.addSingleLineCode},
        ['code-block']: {fn: this.addCodeBlock},
    };

    private static onListing: ListingInterface = null;
    ...
    private createElements() {
        // DownArea
        const downareaElement = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('heading') < 0) {
            // Heading
            const headingTool = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('bold') < 0) {
            // Bold
            const boldTool = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('italic') < 0) {
            // Italic
            const italicTool = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('bold-italic') < 0) {
            // Bold Italic
            const boldItalicTool = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('link') < 0) {
            // Link
            const linkTool = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('image') < 0) {
            // Image
            const imageTool = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('blockquote') < 0) {
            // Blockquote
            const blockquoteTool = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('u-list') < 0) {
            // Unordered List
            const unorderedListTool = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('o-list') < 0) {
            // Ordered List
            const orderedListTool = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('sl-code') < 0) {
            // Single Line Code
            const singleLineCodeTool = document.createElement('div');
        ...
        if (this.hiddenTools.indexOf('code-block') < 0) {
            // Code Block
            const codeBlockTool = document.createElement('div');
    ...
    private listenTextareaEvents() {
        ...
        this.textarea.addEventListener('keydown', (e) => {
            if (e.key.toLowerCase() === 'enter' && DownArea.onListing !== null) {
                ...
                const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
                const end: string = self.textarea.value.substr(self.textarea.selectionStart);
                let splittedStart: string[] = start.split(/\n/g);
                let list: string = '';
                const pattern: RegExp = DownArea.onListing.type === 'unordered' ? /-\s?/ : /\d+\.\s?/;

                if (splittedStart.length) {
                    let match: RegExpMatchArray = splittedStart[splittedStart.length - 1].trim().match(pattern);
    ...
    private setTextareaSize() {
        const downareaRect = this.downareaElement.getBoundingClientRect();
        const toolbarRect = this.toolbar.getBoundingClientRect();
        const bottomRect = this.downareaElement.querySelector('.downarea-bottom').getBoundingClientRect();
    ...
    private initResizer() {
        ...
        const mousedown = (e) => {
            ...
            const rect = self.downareaElement.getBoundingClientRect();
    ...
    private listenTools() {
        ...
        this.tools.map((tool) => {
            tool.addEventListener('click', () => {
                let action = self.actions[tool.dataset.action] ?? null;

                if (action) {
                    if (action.args) {
                        action.fn(self, ...action.args);
                    } else {
                        action.fn(self);
                    }
                }
    ...
    private addTools() {
        const tools = this.downareaElement.querySelectorAll('div.downarea-toolbar *');
        tools.forEach((tool) => {
            if ((tool as HTMLDivElement).dataset.action) {
                this.tools.push(tool as HTMLDivElement);
            }
        });
    }

    private initDropdowns() {
        const tools = this.downareaElement.querySelectorAll('.downarea-toolbar .downarea-toolbar-tool');

        tools.forEach((tool) => {
            // @ts-ignore
            if (tool.dataset.dropdown) {
                // @ts-ignore
                const dropdown = tool.querySelector('#' + tool.dataset.dropdown);
    ...
    private shiftTab(self: DownArea) {
        let start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);
    ...
    public addHeading(self: DownArea, level: number = 1) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);

        let heading: string = `${repeat('#', level)} `;
        let offset: number = 0;

        if (self.textarea.selectionStart != self.textarea.selectionEnd) {
            end = self.textarea.value.substr(self.textarea.selectionEnd);
            const range: string = self.textarea.value.slice(
    ...
    public addBold(self: DownArea) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);

        let bold: string = `****`;
        let offset: number = bold.length - 2;
    ...
    public addItalic(self: DownArea) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);

        let bold: string = `**`;
        let offset: number = bold.length - 1;
    ...
    public addBoldItalic(self: DownArea) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);

        let bold: string = `******`;
        let offset: number = bold.length - 3;
    ...
    public addLink(self: DownArea, type: number = 0) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);
        ...
        if (type === 0) {
            ...
            if (self.textarea.selectionStart != self.textarea.selectionEnd) {
                ...
                const rangeIsURL: RegExpMatchArray = range.match(/https?:\/\/(.+)/);
    ...
    public addEmail(self: DownArea) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);

        let emailLink: string = '<@>';
        let offset: number = emailLink.length - 1;
    ...
    public addImage(self: DownArea) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);

        let link: string = `![](http://)`;
        let offset: number = link.length - 2;
    ...
    public addBlockquote(self: DownArea) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);
    ...
    public addUnorderedList(self: DownArea) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);
    ...
    public addOrderedList(self: DownArea) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);
    ...
    public addSingleLineCode(self: DownArea) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);
    ...
    public addCodeBlock(self: DownArea) {
        const start: string = self.textarea.value.substr(0, self.textarea.selectionStart);
        let end: string = self.textarea.value.substr(self.textarea.selectionStart);
